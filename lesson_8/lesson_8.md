...
- put(key string, value T) -- сохранение значения
- get(key string) T -- получение значения по ключу
- delete(key string) -- удаление значения по ключу
- exist(key string) bool -- проверка на существование
...
type NativeDictionary struct {
	...
	GetGetStatus() Status // успешно; элемента нет в таблице

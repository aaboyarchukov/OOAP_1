...
Спецификация:
- get() T-- возвращает true, если элемент есть в множестве
- set(value T) -- установить элемент в множество
- remove(value T) -- удалить элемент из множества
- union(powerSetFirst PowerSet[T], powerSetSecond PowerSet[T]) PowerSet[T] -- объединение двух множеств
- intersection(powerSetFirst PowerSet[T], powerSetSecond PowerSet[T]) PowerSet[T] -- пересечение двух множеств
- difference(powerSetFirst PowerSet[T], powerSetSecond PowerSet[T]) PowerSet[T] -- разность двух множеств
- issubset(powerSetFirst PowerSet[T], powerSetSecond PowerSet[T]) bool -- проверка, что одно множество содержится в другом
- equals(powerSetFirst PowerSet[T], powerSetSecond PowerSet[T]) bool -- проверка идентичности множеств
...
type PowerSet[T any] struct {
	...
	// precondition: element not in set 
	// postcondition: element in set
	Set(value T)
	
	// precondition: element in set
	// postcondition: element removed from set
	Remove(value T)
	
	// requests
	
	// precondition: element in set
	// postcondiiton: get element from set
	Get(value T) T
	
	// postcondition: get set with all elements from two sets
	Union(powerSetFirst PowerSet[T], powerSetSecond PowerSet[T]) PowerSet[T]
	
	// postcondition: get set with elements from first set which are in second set
	Intersection(powerSetFirst PowerSet[T], powerSetSecond PowerSet[T]) PowerSet[T]
	
	// postcondition: get set with elements from first set which are not in second set
	Difference(powerSetFirst PowerSet[T], powerSetSecond PowerSet[T]) PowerSet[T]
	
	// postcnodition: check if first set contains all elements from second set 
	Issubset(powerSetFirst PowerSet[T], powerSetSecond PowerSet[T]) bool
	
	// postcondition: chech if sets are equal
	Equals(powerSetFirst PowerSet[T], powerSetSecond PowerSet[T]) bool
	
	// additional requests
	GetSetStatus() Status // success; element already in set
	GetRemoveStstua() Status // success; element is are not in set
}
```

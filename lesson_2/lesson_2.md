...
type LinkedListInterface[T any] interface {
	...
	// предусловие: список не пуст
	// постулосвие: курсор встанет на на следующий от себя элемент, который равен значению аргумента
	Find[T](value T)
...
*Потому что она атомарна, так как мы работает с одной сущностью - курсор.* 
